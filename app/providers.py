...
def demo_providers(slow: float = 0.0) -> dict:
    """Deterministic scripted models for the lab-equipment demo."""
    return {
        "supervisor": RoutedMock({
            "Oscilloscope": [
                _call("ask_inventory", question="Is the Digital Oscilloscope available?"),
                _call("ask_booking", request="Book equipment 2 (Digital Oscilloscope) for 2026-09-21 10:00 and message the student to confirm."),
                ModelTurn(text="(mock) The Digital Oscilloscope was available, it is booked for 2026-09-21 10:00, and a confirmation message is queued."),
            ],
            "FPGA": [
                _call("ask_inventory", question="Find the FPGA Development Board"),
                _call("ask_booking", request="Book equipment 3 (FPGA Development Board) for 2026-09-21 11:00."),
                ModelTurn(text="(mock) The FPGA board is available, but the booking was refused because the required lab training is not completed."),
            ],
        }, slow),
        "inventory": RoutedMock({
            "Oscilloscope": [_call("search_equipment", text="Oscilloscope"), ModelTurn(text="(mock) Equipment 2, Digital Oscilloscope: 1 unit available; training required.")],
            "FPGA": [_call("search_equipment", text="FPGA"), ModelTurn(text="(mock) Equipment 3, FPGA Development Board: 1 unit available; training required.")],
        }, slow),
        "booking": RoutedMock({
            "equipment 2": [
                _call("check_can_book", equipment_id=2),
                ModelTurn(text=None, tool_calls=[ToolCall("book_equipment", {"equipment_id":2,"slot":"2026-09-21 10:00"}), ToolCall("notify_student", {"message":"Your Digital Oscilloscope booking for 2026-09-21 10:00 is confirmed."})], tokens_in=150,tokens_out=25),
                ModelTurn(text="(mock) Booked equipment 2 and queued the confirmation."),
            ],
            "equipment 3": [
                _call("check_can_book", equipment_id=3),
                ModelTurn(text="(mock) Not booked: the student has not completed the required lab training."),
            ],
        }, slow),
    }
